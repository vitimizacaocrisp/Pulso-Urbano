...
const hasAttachments = computed(() =>
  !!(analysis.value?.reference_links?.trim())
);
...
<style scoped>
.page-background { background-color: #ffffff; min-height: 100vh; }
.loading-state, .error-state { height: 60vh; display: flex; flex-direction: column; align-items: center; justify-content: center; color: #64748b; }
.spinner { border: 3px solid #e2e8f0; border-top: 3px solid #6366f1; border-radius: 50%; width: 50px; height: 50px; animation: spin 1s linear infinite; margin-bottom: 1rem; }
@keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }
.error-box { text-align: center; background: #fef2f2; padding: 2rem; border-radius: 12px; color: #991b1b; }
.error-box i { font-size: 2rem; margin-bottom: 1rem; }
.btn-secondary { display: inline-block; margin-top: 1rem; color: #991b1b; text-decoration: underline; }
...
.hero-category { display: inline-block; background: #6366f1; color: white; padding: 0.25rem 0.75rem; border-radius: 4px; font-size: 0.8rem; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 1rem; }
...
.back-only-bar { padding: 1rem 1.5rem; background: #f8fafc; border-bottom: 1px solid #e2e8f0; }
.back-link-plain { background: none; border: 1px solid #cbd5e1; color: #334155; padding: 0.4rem 1rem; border-radius: 20px; cursor: pointer; display: inline-flex; align-items: center; gap: 0.5rem; font-size: 0.9rem; transition: background 0.2s, color 0.2s; }
.back-link-plain:hover { background: #e2e8f0; color: #0f172a; }
.article-body-wrapper { background: white; position: relative; z-index: 3; padding-bottom: 4rem; }
...
.attachments-section { margin-top: 4rem; background: #f8fafc; border-radius: 12px; padding: 2rem; border: 1px solid #e2e8f0; }
.section-heading { font-size: 1.25rem; color: #0f172a; margin-bottom: 1.5rem; padding-bottom: 0.5rem; border-bottom: 2px solid #cbd5e1; display: inline-block; }
.attachments-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 1.5rem; }
.attach-card h4 { font-size: 0.9rem; text-transform: uppercase; color: #64748b; margin-bottom: 1rem; font-weight: 700; }
.link-list     { list-style: none; padding: 0; }
.link-list li  { margin-bottom: 0.75rem; }
.link-list a   { display: flex; align-items: center; gap: 0.5rem; text-decoration: none; color: #334155; font-size: 0.95rem; padding: 0.5rem; border-radius: 6px; transition: background 0.2s; }
.link-list a:hover { background: #e2e8f0; color: #2563eb; }
