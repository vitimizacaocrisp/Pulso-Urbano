    ...
    <div class="layout-wrapper">
      ...
      <div class="main-content">
        ...
        <div class="controls-bar">
          <div class="search-field-wrapper">
            ...
            <input
              ...
              type="text"
              v-model="searchQuery"
              placeholder="Título, ID, fonte, tag, texto..."
        ...
        <div class="active-pills" v-if="hasActiveFilters">
          <span v-for="cat in activeFilters.categories" :key="'c-'+cat" class="pill" @click="removeFilter('categories', cat)">
            {{ cat }} <i class="fas fa-times"></i>
          </span>
          <span v-for="src in activeFilters.sources" :key="'s-'+src" class="pill" @click="removeFilter('sources', src)">
            {{ src }} <i class="fas fa-times"></i>
          </span>
          <span v-for="tag in activeFilters.tags" :key="'t-'+tag" class="pill" @click="removeFilter('tags', tag)">
        ...
        <div v-else class="results-grid">
          ...
          >
            <router-link
              ...
              v-if="analysis.cover_image_path"
            >
            ...
            <div v-else class="card-no-image">
              <span>{{ getInitials(analysis.title) }}</span>
            </div>
            ...
            <div class="card-body">
              ...
              <div class="card-footer">
                <span class="card-source" v-if="analysis.source">
                  <i class="fas fa-landmark"></i> {{ analysis.source }}
                </span>
...
// Opções disponíveis (populadas após primeiro fetch)
...
const activeFilterCount = computed(() => {
  let n = activeFilters.value.categories.length
        + activeFilters.value.sources.length
...
// ─── Scoring de relevância (client-side) ────────────────────────────
/**
 * Pontua cada análise com base em quão bem ela bate com a query.
 * Campos mais específicos (título, id) têm peso maior.
 */
const scoreAnalysis = (a, q) => {
  ...
  const check = (field, weight) => {
    ...
    terms.forEach(t => {
      if (f === t) score += weight * 2;         // match exato
  ...
  check(a.source,       5);
  check(a.author,       4);
  check(a.study_period, 3);
  check(a.description,  2);
  check(a.content,      1);
  return score;
};
...
const fetchAnalyses = async (isNewSearch = false) => {
  ...
  try {
    ...
    const params = {
      ...
      limit:      limit.value,
      category:   activeFilters.value.categories.join(',') || undefined,
      source:     activeFilters.value.sources.join(',')    || undefined,
      tag:        activeFilters.value.tags.join(',')       || undefined,
      year_from:  activeFilters.value.yearFrom             || undefined,
      year_to:    activeFilters.value.yearTo               || undefined,
    ...
    const response = await axios.get(`${API_BASE_URL}/api/admin/analyses-list`, { headers, params });
    const result = response.data;
    let fresh = result.data?.analyses || [];

    // Ordenação por relevância client-side quando sortBy === 'relevance' e há query
    if (sortBy.value === 'relevance' && searchQuery.value.trim()) {
      fresh = fresh
        .map(a => ({ ...a, _score: scoreAnalysis(a, searchQuery.value) }))
    ...
    // Popula opções de filtro na primeira carga
    if (isNewSearch && availableCategories.value.length === 0) {
...
const populateFilterOptions = async () => {
  try {
    const token = localStorage.getItem('authToken');
    const headers = token ? { Authorization: `Bearer ${token}` } : {};
    const r = await axios.get(`${API_BASE_URL}/api/admin/analyses-list`, {
      headers,
      params: { limit: 'all' }
    });
    const all = r.data?.data?.analyses || [];
    ...
    all.forEach(a => {
      ...
      processTags(a.tag).forEach(t => tagSet.add(t));
    });

    availableCategories.value = [...catSet].sort();
    availableSources.value    = [...srcSet].sort();
    availableTags.value       = [...tagSet].sort().slice(0, 40);
  } catch (_) { /* silencioso */ }
};
...
const clearAllFilters = () => {
  ...
  activeFilters.value = { categories: [], sources: [], tags: [], yearFrom: null, yearTo: null };
  applyFilters();
};

const clearPeriod = () => {
  activeFilters.value.yearFrom = null;
  activeFilters.value.yearTo   = null;
  applyFilters();
};

const removeFilter = (type, val) => {
  activeFilters.value[type] = activeFilters.value[type].filter(v => v !== val);
  applyFilters();
};

const toggleTag = (tag) => {
  const idx = activeFilters.value.tags.indexOf(tag);
  if (idx === -1) activeFilters.value.tags.push(tag);
  else activeFilters.value.tags.splice(idx, 1);
  applyFilters();
};
...
.state-container p { font-size: 1rem; }
.retry-btn {
  background: var(--brand-primary);
  border: none;
  color: #fff;
